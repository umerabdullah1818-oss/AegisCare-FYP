...
import { Heart, Activity, User, TrendingUp, TrendingDown, Bell, Pill, ActivitySquare, Wind, Utensils, ShieldAlert, Zap, BedDouble, Droplets } from 'lucide-react';
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className={`lg:col-span-2 rounded-2xl p-6 backdrop-blur-xl border-2 ${
                  ...
                  {/* Animated Chart */}
                  <AnimatedChart isDarkMode={isDarkMode} />
                ...
                <div className={`rounded-2xl p-6 backdrop-blur-xl border-2 ${
                  ...
                  <div className="space-y-3">
                    {(() => {
                      ...
                      // Heart rate activity
                      if (hrStatus !== 'Normal') {
                        activities.push({ time: '2 min ago', activity: `Heart rate ${hr > 100 ? 'spiked' : 'dropped'} to ${hr} BPM`, status: 'warning', icon: <Heart className="w-4 h-4" /> });
                      } else {
                        activities.push({ time: '2 min ago', activity: `Heart rate stable at ${hr} BPM`, status: 'success', icon: <Heart className="w-4 h-4" /> });
                      }
                      // Medication activity
                      ...
                      if (glStatus === 'Normal') {
                        ...
                        activities.push({ time: '3 hours ago', activity: `Blood glucose elevated at ${gl} mg/dL — monitor diet`, status: 'warning', icon: <ActivitySquare className="w-4 h-4" /> });
                      }
                      // BP activity
                      if (bpStatus === 'High') {
                        activities.push({ time: '4 hours ago', activity: `Blood pressure high at ${sbp}/${dbp} mmHg — alert sent to doctor`, status: 'warning', icon: <Activity className="w-4 h-4" /> });
                      }
                      // SpO2 activity
                      if (spStatus !== 'Excellent') {
                        activities.push({ time: '5 hours ago', activity: `SpO2 at ${sp}% — ${sp < 90 ? 'critically low, seek help' : 'below normal, monitor'}`, status: 'warning', icon: <Wind className="w-4 h-4" /> });
                      }
                      return activities;
                    })().map((activity, idx) => (
