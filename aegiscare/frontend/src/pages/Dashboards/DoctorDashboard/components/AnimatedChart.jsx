...
const AnimatedChart = ({ data, labels, colors = ['#3b82f6', '#10b981', '#8b5cf6'], height = 200, type = 'line', isDarkMode }) => {
    // Determine bounds and add 10% visual padding above max value
    const maxValue = Math.max(...data.flat(), 100) * 1.1; 
    
    // Helper to normalize data points to a 0-100 scale for SVG rendering
    const norm = (val) => (val / maxValue) * 100;
            ...
            {data.map((dataset, idx) => {
              if (type === 'line') {
                return (
                  <path
                    key={idx}
                    d={`M0,${100 - norm(dataset[0])} ${dataset.map((val, i) => `L${(i / (dataset.length - 1)) * 100},${100 - norm(val)}`).join(' ')}`}
                    fill="none"
